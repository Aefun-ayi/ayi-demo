...
class Out_Ad():
    ...
    def connect_phone(self, devices_id):
        self.dirver = u2.connect(devices_id)
        return self.dirver.device_info

    def start_app(self, devices_id):
        self.dirver = u2.connect(devices_id)
    ...
    def stop_app(self, devices_id):
        os.popen(f'adb -s {devices_id} shell am force-stop {self.package}')
