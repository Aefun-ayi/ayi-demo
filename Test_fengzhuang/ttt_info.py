...
class Main():
    ...
    def ttt_test(self):
        appid = ui.app_on_id.text()
        cha = ui.app_on_chanl.text()
        apk_version = ui.apk_on_version.text()
        track_channel = ui.track_channel.currentText()
        ttt_cfg = ttt_online.ttt_test(appid,cha,apk_version,track_channel)
    ...
    def tta_release(self):
        ...
        apk_version = ui.apk_on_version.text()
        track_channel = ui.track_channel.currentText()
        tta_cfg = tta_online.tta_release(appid,cha,apk_version,track_channel)
    ...
    def ttt_ad(self):
        ...
        tttad = ttt_ad.ttt_ad(new_or_old,appid,out_pack)
    ...
    def tta_ad(self):
        ...
        if peop == '老用户':
            new_or_old = new_old.replace('待替换','2')
        ttaad = tta_ad.tta_ad(new_or_old,appid,out_pack)
