...
def tta_ad(new_or_old, appid, out_pack):
    splash_name = []
    splash_source = []
    video_name = []
    video_source = []
    ...
    url = 'http://192.168.7.30:8009/tta_spaceid'
    res = requests.get(url)
    ad_info = res.json()[f'{out_pack}']
    url = 'https://tta.careduka.com/space-slot/getSlotList'
    head = {
            ...
            for i in range(len(slot)):
                ...
                if slot[i]['ad_type'] == 1:
                    for ii in range(len(bidding)):
                        ...
                        splash_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：bidding-{bidding_source}/代码位id：{bidding_id}/")
                    ...
                    if len(waterfall) > 0:
                        for iii in range(len(waterfall)):
                            ...
                            splash_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：waterfall-{waterfall_source}/代码位id：{waterfall_id}/")
                if slot[i]['ad_type'] == 2:
                    for ii in range(len(bidding)):
                        bidding_source = bidding[ii]['rit_name']
                        bidding_id = bidding[ii]['rit_id']
                        video_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：bidding-{bidding_source}/代码位id：{bidding_id}/")
                    waterfall = slot[i]['rit']['price']
                    if len(waterfall) > 0:
                        for iii in range(len(waterfall)):
                            waterfall_source = waterfall[iii]['rit_name']
                            waterfall_id = waterfall[iii]['rit_id']
                            video_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：waterfall-{waterfall_source}/代码位id：{waterfall_id}/")
                if slot[i]['ad_type'] == 3:
                    for ii in range(len(bidding)):
                        bidding_source = bidding[ii]['rit_name']
                        bidding_id = bidding[ii]['rit_id']
                        msg_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：bidding-{bidding_source}/代码位id：{bidding_id}/")
                    waterfall = slot[i]['rit']['price']
                    if len(waterfall) > 0:
                        for iii in range(len(waterfall)):
                            waterfall_source = waterfall[iii]['rit_name']
                            waterfall_id = waterfall[iii]['rit_id']
                            msg_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：waterfall-{waterfall_source}/代码位id：{waterfall_id}/")
                if slot[i]['ad_type'] == 4:
                    for ii in range(len(bidding)):
                        bidding_source = bidding[ii]['rit_name']
                        bidding_id = bidding[ii]['rit_id']
                        plaque_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：bidding-{bidding_source}/代码位id：{bidding_id}/")
                    waterfall = slot[i]['rit']['price']
                    if len(waterfall) > 0:
                        for iii in range(len(waterfall)):
                            waterfall_source = waterfall[iii]['rit_name']
                            waterfall_id = waterfall[iii]['rit_id']
                            plaque_source.append(f"space_id：{res.json()['data']['space_id']}/代码位名称：waterfall-{waterfall_source}/代码位id：{waterfall_id}/")
